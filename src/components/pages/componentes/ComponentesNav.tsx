      ...
      <nav
        ...
        {items.map((item) => (
          <a
            key={item.id}
            href={`#${item.id}`}
            className='shrink-0 rounded-lg border border-border bg-surface px-4 py-2 text-sm font-medium text-foreground transition-colors hover:border-primary hover:text-primary lg:shrink lg:px-3 lg:py-2'>
            {item.label}
          </a>
        ))}
      </nav>
